...
from django_user_log.context import get_user_attrs
...
def test_default_settings(settings: SettingsWrapper) -> None:
    ...
    del settings.DJANGO_USER_LOG

    # Instantiate a new django-user-log settings instance
    Settings()
...
def test_user_defined_settings(settings: SettingsWrapper) -> None:
    ...
    for setting, value in settings.DJANGO_USER_LOG.items():
...
def test_invalid_user_defined_setting(settings: SettingsWrapper) -> None:
    ...
    with pytest.raises(ImproperlyConfigured, match="'INVALID_OPTION' is not a valid django-user-log setting."):
        Settings()
...
def test_user_attr_import_string(settings: SettingsWrapper) -> None:
    ...
    settings.DJANGO_USER_LOG = {"USER_ATTRS": {"username": "ext://django_user_log.context.get_user_attrs"}}
...
@pytest.mark.parametrize(
    ...
    [
        ("<class 'bool'>", False),
        ("<class 'float'>", 3.14),
        (r"<class '.*ContextVar'>", "ext://django_user_log.context.user_attrs"),
    ],
)
def test_invalid_user_attr_type(type_name: str, value: Any, settings: SettingsWrapper) -> None:
    ...
    settings.DJANGO_USER_LOG = {"USER_ATTRS": {"username": value}}
